...
mysqldump -u$user -p$userPWD --opt --default-character-set=utf8 --extended-insert=false --triggers -R --hex-blob --all-databases --flush-logs -x > $dumpFile
...
chmod u+r bak
corntabl -e
30 1 * * 6 root mysqldump -u root -pPASSWORD --all-databases | gzip > /Data/bakup/database_`date '+%m-%d-%Y'`.sql.gz
