...
&&yum install cmake -y ；cmake . -DCMAKE_INSTALL_PREFIX=$M_PREFIX \ -DMYSQL_UNIX_ADDR=/tmp/mysql.sock \ -DMYSQL_DATADIR=/data/mysql \ -DSYSCONFDIR=/etc \ -DMYSQL_USER=mysql \ -DMYSQL_TCP_PORT=3306 \
...
if [ $? -eq 0 ]；then
make && make install
echo -e "\n\033[32m-----------------------------------------------\033[0m" echo -e "\033[32mThe $M_FILES_DIR Server Install
Success !\033[0m" else
echo -e "\033[32mThe $M_FILES_DIR Make or Make install
ERROR,Please Check......" exit 0
fi
fi
#Install PHP server
if [[ "$1" -eq "3" ]]；then
...
cho "You can access http://$IP/" cat >$H_PREFIX/htdocs/index.php <<EOF
<?php
phpinfo()；
?>
EOF
